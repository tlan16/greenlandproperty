...
function aviators_properties_load_styles() {
  if (is_admin()) {
    ...
    wp_enqueue_script('geolocation', get_template_directory_uri() . '/aviators/plugins/properties/assets/js/geolocation.js');
  }
}
  ...
  switch ($column) {
    case 'thumbnail':
      if (has_post_thumbnail($post_id)) {
        ...
        echo '<img src="' . get_template_directory_uri() . '/assets/img/property-tmp-small.png' . '" width="80">';
      }
      break;
    case 'optional_title':
...
function aviators_properties_property_get_actions($post_id) {
  ...
  $links[] = sprintf("<a title=\"%s\" class=\"edit\" href=\"%s\">%s</a>",
    _aviators_properties_submission_create_link(array('action' => 'edit', 'id' => $post_id)),
    _aviators_properties_submission_create_link(array('action' => 'edit', 'id' => $post_id)),
    __('Edit', 'aviators'));

  $links[] = sprintf("<a title=\"%s\" class=\"remove\" href=\"%s\">%s</a>",
    _aviators_properties_submission_create_link(array('action' => 'delete', 'id' => $post_id)),
    _aviators_properties_submission_create_link(array('action' => 'delete', 'id' => $post_id)),
    __('Delete', 'aviators'));

  $links[] = sprintf("<a title=\"%s\" class=\"view\" href=\"%s\">%s</a>",
    get_permalink($post_id),
    get_permalink($post_id),
    __('View', 'aviators'));

  $publish_link = sprintf("<a title=\"%s\" class=\"publish\" href=\"%s\">%s</a>'",
  ...
  $unpublish_link = sprintf("<a title=\"%s\" class=\"unpublish\" href=\"%s\">%s</a>'",
    ...
    switch ($action) {
      case 'publish':
        ...
        if ($paypal_condition && !$is_paid) {
          aviators_flash_add_message(AVIATORS_FLASH_ERROR, __('You need to pay for submission item in order to publish it', 'aviators'));
          $page = _aviators_properties_get_submission_page();
          wp_redirect(get_permalink($page));
          return false;
        }
        ...
        $bool = aviators_settings_get_value('properties', 'common', 'frontend_needs_submission');
        if($bool) {
          aviators_flash_add_message(AVIATORS_FLASH_ERROR, __('Administrator approval is required to publish the post', 'aviators'));
        }
        break;
      case 'unpublish':
        /**
...
function aviators_properties_property_edit($id = NULL, $values) {
  ...
  update_post_meta($id, '_property_meta_fields', $meta_fields);
...
function aviators_properties_property_delete($id) {
  ...
  aviators_flash_add_message(AVIATORS_FLASH_SUCCESS, __("Are you sure you want to delete this post? <a href=\"$link\" class=\"btn\">Yes, delete</a>", 'aviators'));
}
